...
CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ui_original.yml")
...
class SlayerBridge:
    ...
    def launch_slayer_job(self, settings):
        ...
        # Build command
        cmd = [sys.executable, 'watermark_slayer_original.py', input_path, output_path]
    ...
    def _stream_worker(self, cmd):
        ...
        try:
            ...
            cli_display = cli_display.replace('watermark_slayer_original.py ', 'python watermark_slayer_original.py \\\n    ')
            ...
            script_path = os.path.join(working_dir, 'watermark_slayer_original.py')

            # Verify script exists
            if not os.path.exists(script_path):
                self._send_frontend_event(f'addLog("ERROR: watermark_slayer_original.py not found at {json.dumps(script_path)}", "text-error")')
    ...
    def preview_slayer_detection(self, settings):
        ...
        try:
            ...
            cmd = [
                sys.executable, 'watermark_slayer_original.py',
                input_path, '--preview',
...
def launch_gui():
    ...
    ui_path = os.path.join(script_dir, 'ui_original', 'index.html')
